...
from app.helpers import FileSize
...
class WorkForm(FlaskForm):
    ...
    content_category = SelectField('Тип контента',
    ...
    url = StringField('URL',
                      validators=[Optional(), URL(message="Некорректный URL")])

    file = FileField('Файл', validators=[
        Optional(),
        FileAllowed(['pdf', 'png', 'jpg', 'jpeg'],
                    'Допустимы только PDF, PNG, JPG/JPEG')
    ])
    ...
    category = SelectField('Категория',
                           choices=DEFAULT_CATEGORIES, validators=[Optional()])
    custom_category = StringField('Или введите свою категорию',
                                  validators=[Optional(), Length(max=50)])
    markers = StringField('Маркеры',
                          validators=[Optional()])
    ...
    def validate(self, extra_validators=None):
        ...
        if content_type == 'video':
            if not is_edit and not self.video_file.data:
                ...
                return False
            if (is_edit and not self.video_file.data
                    and not self.work.video_url):
                (self.video_file.errors.append
                 ('Загрузите видеофайл или используйте текущий'))
                return False
            ...
            if is_edit and not self.file.data and not self.work.file_url:
                (self.file.errors.append
                 ('Загрузите файл или используйте текущий'))
                return False
...
class ProfileEditForm(FlaskForm):
    ...
    last_name = StringField('Фамилия',
                            validators=[DataRequired(), Length(max=50)])
    current_password = PasswordField('Текущий пароль (для изменений)',
                                     validators=[Optional()])
    ...
    avatar = FileField('Аватар', validators=[
        ...
        FileSize(max_size=5 * 1024 * 1024, message='Макс. размер 5 МБ')
    ])
